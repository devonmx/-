...
$('#calcular').click(function(){
	...
	$.map(salarios, function(jugador, index){
		...
		salarios[index]['goles_minimos'] = minimoGoles;
		...
		salarios[index]['alcance_individual'] = alcanceIndividual;
	});
	...
	$.map(salarios, function(jugador, index){
		...
		porcentajeBono = (salarios[index]['alcance_individual'] + calcularAG) / 2;

		// Calculamos el bono alcanzado respecto a la media obtenida
		bonoAlcanzado = (porcentajeBono * salarios[index]['bono']) / 100;

		// Agregamos al array el sueldo completo
		salarios[index]['sueldo_completo'] = salarios[index]['sueldo'] + bonoAlcanzado;
